...
size="$(stat -f '%z' "$FILE")"; last=$((size - 1)); checksum="$(openssl dgst -sha256 -binary "$FILE" | base64 | tr -d '\n')"; name="$(basename "$FILE")"
payload="$(jq -cn --arg org "$POLAR_ORGANIZATION_ID" --arg name "$name" --arg version "$VERSION" --arg checksum "$checksum" --argjson size "$size" --argjson last "$last" '{organization_id:$org,name:$name,mime_type:"application/x-apple-diskimage",size:$size,checksum_sha256_base64:$checksum,service:"downloadable",version:$version,upload:{parts:[{number:1,chunk_start:0,chunk_end:$last,checksum_sha256_base64:$checksum}]}}')"
...
etag="$(awk 'BEGIN{IGNORECASE=1} /^etag:/{gsub(/\r|\"/,""); gsub(/^etag:[[:space:]]*/,""); print; exit}' "$TMP/headers")"; [ -n "$etag" ] || exit 1
